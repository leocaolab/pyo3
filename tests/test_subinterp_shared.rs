...
fn shared_extension() {
    ...
    let main_module = Python::attach(|py| pyo3::wrap_pymodule!(shared_probe)(py).as_ptr() as usize);
    let sub_module =
        in_sub_interpreter(|py| pyo3::wrap_pymodule!(shared_probe)(py).as_ptr() as usize);
    assert_ne!(
        sub_module, main_module,
        "a sub-interpreter got the main interpreter's module object"
    );
    ...
    let attached = std::thread::spawn(|| Python::try_attach(|_| ()))
        .join()
        .unwrap();
    assert!(
        attached.is_none(),
        "an ambiguous foreign-thread attach must fail, not land in main"
    );
    let msg = panic_message(
        std::thread::spawn(|| Python::attach(|_| ()))
            .join()
            .unwrap_err(),
    );
    assert!(
        msg.contains("more than one interpreter"),
        "unexpected attach panic: {msg}"
    );
    ...
    assert!(
        msg.contains("cannot tell which interpreter owns"),
        "unexpected drop panic: {msg}"
    );
