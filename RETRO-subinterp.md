...
> **更正(2026-09-22,#8):上面这句关于驻留字符串的判断是错的,而且当时没量。**
> 实测:运行时驻留的字符串(`sys.intern("".join([...]))`,和 PyO3 的 `PyString::intern`
> 同一条路径)在 3.14.7 上 `sys._is_immortal(s) == False`,主解释器和子解释器都是;
> 3.12.13、3.13.15 上 refcount 为 2,同样**不是不朽的**。所以进程级的 `intern!` 让所有
> 子解释器共用第一个解释器的字符串:引用计数在不同的 GIL 下被改写,那个解释器销毁后还是
> 悬空指针。已在 `3c5d769`(#5)改成每解释器一份,`cache_probe.py` 上游 1/4 → 本分支 4/4。
...
是不朽对象吗?       是 → 安全(CPython 静态类型)—— 要【量】,不要按类别推断
否则                 → ★ 危险 —— 任何非不朽对象都要改,不只是堆类型
```

> **更正(2026-09-22,#8):** 原判据把"3.12+ 驻留字符串"列为不朽,并写着"是堆类型吗 → 只有这一类要改"。
> 两处都错:驻留字符串实测不是不朽的(见上),而它也不是堆类型 —— 危险的判据是"非不朽",
> 不是"堆类型"。判定是否不朽要用 `sys._is_immortal` 实测。
