...
polars 上的压测(345,600 次写 / 113 万次操作 / 4–12 个子解释器)找出四个问题,
**其中最重的一个在这个分支自己的代码域内**。详见
[`subinterp-bench/POLARS.md`](subinterp-bench/POLARS.md) 的"压测"一节。

```
★ A  pyo3 的全局延迟释放池跨解释器 free      N=8 写出,1–4 秒内 6/6 SIGABRT
     static POOL: OnceLock<ReferencePool>   进程级,不分解释器
     解释器 A 延迟释放的对象被 B free 掉,同时无同步地改 A 的引用计数
★ B  map_elements 的 return_dtype 串号       N=2 时 50%,静默给出【别的解释器的类对象】
     上游在同配置下是【大声报错】,这个分支变成【静默给错数据】—— 更糟
  C  ≥2 个解释器算过之后销毁会段错误          归因最弱,可能是 CPython 自身
  D  每个子解释器生命周期泄漏 ~60KB           legacy 配置 A/B 干净归因到本分支
     且同负载 import 慢 3.7 倍
```

**A 这条最该记:** 这个分支把 `#[pyclass]` 类型对象、异常类型、模块对象、file-like
回调都做成了每解释器一份,**唯独这个池没有**。而 `AssumeAttached` 的文档注释里
写过这个危险 —— 识别了陷阱、在一处绕开了、没有追到根上。

**结论要相应改口:这个分支现在能证明的是"能进去、隔离对、扩展好、单解释器下正确",
不是"能上生产"。**
